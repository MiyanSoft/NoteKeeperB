...
public class NoteKeeperProvider extends ContentProvider {
    ...
    //NO_MATCH indicates that any attempt to match URI that doesn't contain an authority or a path should return value NO_MATCH
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (uriMatch)   {
            ...
            case COURSES_ROW:
              ...
              selection = CourseInfoEntry._ID + " =?";
              selectionArgs = new String[] {Long.toString(rowId)};
              nRows = db.delete(CourseInfoEntry.TABLE_NAME, selection, selectionArgs);
              break;
            case NOTES_ROW:
              rowId = ContentUris.parseId(uri);
              selection = NoteInfoEntry._ID + " =?";
              selectionArgs = new String[] {Long.toString(rowId)};
              nRows = db.delete(NoteInfoEntry.TABLE_NAME, selection, selectionArgs);
              break;
        }
    ...
    public String getType(Uri uri) {
        ...
        switch (uriMatch)   {
            case COURSES:
                //vnd.android.cursor.dir/vnd.com.example.provider.courses
                mimeType = ContentResolver.CURSOR_DIR_BASE_TYPE + "/" + MIME_VENDOR_TYPE + Courses.PATH;
                break;
            case NOTES:
                mimeType = ContentResolver.CURSOR_DIR_BASE_TYPE + "/" + MIME_VENDOR_TYPE + Notes.PATH;  //returns multiple row
                break;
            case NOTES_EXPANDED:
                mimeType = ContentResolver.CURSOR_DIR_BASE_TYPE + "/" + MIME_VENDOR_TYPE + Notes.PATH_EXPANDED;
                break;
            case NOTES_ROW:
                mimeType = ContentResolver.CURSOR_ITEM_BASE_TYPE + "/" + MIME_VENDOR_TYPE + Notes.PATH; //returns single row
                break;
            case COURSES_ROW:
